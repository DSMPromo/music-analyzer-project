...
function ChordDetector({
  ...
  // === CIRCLE ANIMATIONS WITH DECAY ===
  // Track intensity levels (0-1) for smooth decay animations
  const [chordIntensity, setChordIntensity] = useState(0);
  const [drumIntensities, setDrumIntensities] = useState({});
  const lastChordRef = useRef(null);
  const lastDrumTimesRef = useRef({});
  const animationFrameRef = useRef(null);

  // Decay constants
  const DECAY_RATE = 0.92; // How fast intensity decays (0.9 = fast, 0.98 = slow)
  const MIN_INTENSITY = 0.05; // Below this, set to 0
  const TRIGGER_INTENSITY = 1.0; // Initial intensity on trigger
  ...
  useEffect(() => {
    ...
    const animate = (currentTime) => {
      ...
      // Decay chord intensity
      setChordIntensity(prev => {
        const newIntensity = prev * Math.pow(DECAY_RATE, deltaTime);
        return newIntensity < MIN_INTENSITY ? 0 : newIntensity;
      });

      // Decay drum intensities
      setDrumIntensities(prev => {
        const updated = { ...prev };
        let hasChanges = false;
        Object.keys(updated).forEach(drum => {
          const newIntensity = updated[drum] * Math.pow(DECAY_RATE, deltaTime);
          if (newIntensity < MIN_INTENSITY) {
            delete updated[drum];
            hasChanges = true;
          } else if (Math.abs(updated[drum] - newIntensity) > 0.001) {
            updated[drum] = newIntensity;
            hasChanges = true;
          }
        });
  ...
  // Trigger chord intensity on chord change
  useEffect(() => {
    if (currentChord && currentChord.symbol !== lastChordRef.current) {
      setChordIntensity(TRIGGER_INTENSITY);
      lastChordRef.current = currentChord.symbol;
    }
  }, [currentChord]);

  // Trigger drum intensities on drum hits
  useEffect(() => {
    if (!drumHits || !isPlaying) return;

    const now = currentTimeMs;
    const triggerWindow = 50; // ms window for triggering

    Object.entries(drumHits).forEach(([drumType, hits]) => {
      if (!hits || !Array.isArray(hits)) return;

      const lastTriggerTime = lastDrumTimesRef.current[drumType] || 0;

      // Find a recent hit that we haven't triggered yet
      const newHit = hits.find(hit => {
        const hitTime = hit.timestamp;
        return hitTime > lastTriggerTime &&
               hitTime <= now &&
               hitTime > now - triggerWindow;
      });

      if (newHit) {
        lastDrumTimesRef.current[drumType] = newHit.timestamp;
        setDrumIntensities(prev => ({
          ...prev,
          [drumType]: TRIGGER_INTENSITY
        }));
      }
    });
  }, [drumHits, currentTimeMs, isPlaying]);
  ...
  // Render Harmonic Circle of Fifths (LEFT - for chords) with decay animation
  const renderHarmonicCircle = () => {
    ...
    // Calculate glow intensity based on decay
    const glowIntensity = chordIntensity;
    const pulseScale = 1 + (glowIntensity * 0.15); // Max 15% scale increase
    const glowSize = 6 + (glowIntensity * 12); // 6-18px glow
    ...
    return (
      <svg width="190" height="190" className="circle-of-fifths harmonic-circle" data-testid="circle-of-fifths">
        ...
        {CIRCLE_OF_FIFTHS.map((note, i) => {
          ...
          const baseRadius = 14;
          const activeRadius = isActive ? baseRadius + 2 + (glowIntensity * 4) : baseRadius;

          return (
            <g key={note} style={{ transition: 'transform 0.05s ease-out' }}>
              {/* Glow effect for active note */}
              {isActive && glowIntensity > 0 && (
                <circle
                  cx={pos.x}
                  cy={pos.y}
                  r={activeRadius + glowSize}
                  fill="#e94560"
              ...
              <circle
                ...
                stroke={isActive ? '#ff6b8a' : isRelated ? '#3b82f6' : '#0f3460'}
                strokeWidth={isActive ? 3 : isRelated ? 2 : 1}
                style={{
                  filter: isActive ? `drop-shadow(0 0 ${glowSize}px #e94560)` : 'none',
              ...
              <text
                ...
                dy="4"
                fill="white"
                fontSize={isActive ? 11 + glowIntensity * 2 : 10}
                fontWeight={isActive ? 700 : 500}
                style={{ transition: 'font-size 0.05s ease-out' }}
              >
                {note}
              </text>
            </g>
        ...
        {MINOR_CIRCLE.map((note, i) => {
          ...
          const baseRadius = 10;
          const activeRadius = isActive ? baseRadius + 2 + (glowIntensity * 3) : baseRadius;

          return (
            <g key={note}>
              {/* Glow effect for active minor */}
              {isActive && glowIntensity > 0 && (
                <circle
                  cx={pos.x}
                  cy={pos.y}
                  r={activeRadius + glowSize * 0.7}
                  fill="#a855f7"
                  opacity={glowIntensity * 0.4}
                  style={{ transition: 'all 0.05s ease-out' }}
                />
              ...
              <circle
                ...
                stroke={isActive ? '#c084fc' : '#16213e'}
                strokeWidth={isActive ? 2 : 1}
                style={{
                  filter: isActive ? `drop-shadow(0 0 ${glowSize * 0.8}px #a855f7)` : 'none',
                  transition: 'all 0.05s ease-out'
                }}
              />
        ...
        {/* Center - chord symbol with pulse */}
        <text
          x={centerX}
          y={centerY + 5}
          textAnchor="middle"
          fill="white"
          fontSize={18 + glowIntensity * 4}
          fontWeight="bold"
          style={{
            filter: glowIntensity > 0 ? `drop-shadow(0 0 ${glowIntensity * 8}px ${isMinor ? '#a855f7' : '#e94560'})` : 'none',
            transition: 'all 0.05s ease-out'
          }}
        >
          {currentChord?.symbol || '—'}
  ...
  // Render Rhythmic Circle (RIGHT - for drums) with decay animation
  const renderRhythmicCircle = () => {
    ...
    // Get drum type and intensity for a note
    const getDrumForNote = (note) => {
      for (const [drumType, drumNote] of Object.entries(DRUM_NOTE_MAP)) {
        if (drumNote === note) {
          return {
            drumType,
            intensity: drumIntensities[drumType] || 0,
            isActive: activeDrums[drumType] || drumIntensities[drumType] > 0
          };
        }
      }
      return null;
    ...
    // Calculate overall activity for background pulse
    const totalIntensity = Object.values(drumIntensities).reduce((sum, i) => sum + i, 0);
    const avgIntensity = Object.keys(drumIntensities).length > 0
      ? totalIntensity / Object.keys(drumIntensities).length
      : 0;
    ...
    return (
      <svg width="190" height="190" className="circle-of-fifths rhythmic-circle" data-testid="rhythmic-circle">
        ...
        <circle
          cx={centerX}
          cy={centerY}
          r={radius + 12 + (avgIntensity * 8)}
          fill="none"
          stroke="#22c55e"
          strokeWidth={2 + avgIntensity * 2}
          opacity={0.3 + avgIntensity * 0.5}
          style={{ transition: 'all 0.05s ease-out' }}
        />
        ...
        {CIRCLE_OF_FIFTHS.map((note, i) => {
          const pos = getPosition(i, radius);
          const drumInfo = getDrumForNote(note);
          const hasDrumMapping = drumInfo !== null;
          const intensity = drumInfo?.intensity || 0;
          const drumColor = hasDrumMapping ? DRUM_COLORS[drumInfo.drumType] : null;

          // Dynamic sizing based on intensity
          const baseRadius = 14;
          const activeRadius = intensity > 0 ? baseRadius + 4 + (intensity * 6) : baseRadius;
          const glowSize = 8 + (intensity * 14);

          return (
            <g key={note}>
              {/* Glow effect for active drums */}
              {intensity > 0 && drumColor && (
                <circle
                  cx={pos.x}
                  cy={pos.y}
                  r={activeRadius + glowSize}
                  ...
                  style={{ transition: 'all 0.03s ease-out' }}
                />
              )}
              <circle
                cx={pos.x}
                cy={pos.y}
                r={activeRadius}
                fill={intensity > 0 ? drumColor : hasDrumMapping ? '#1a1a2e' : '#0f0f1a'}
                stroke={intensity > 0 ? drumColor : hasDrumMapping ? '#333' : '#222'}
                strokeWidth={intensity > 0 ? 3 : 1}
                opacity={hasDrumMapping ? 1 : 0.3}
                style={{
                  filter: intensity > 0 ? `drop-shadow(0 0 ${glowSize}px ${drumColor})` : 'none',
              ...
              <text
                ...
                textAnchor="middle"
                dy="4"
                fill={intensity > 0 ? 'white' : hasDrumMapping ? '#888' : '#444'}
                fontSize={intensity > 0 ? 11 + intensity * 2 : 10}
                fontWeight={intensity > 0 ? 700 : 400}
                style={{ transition: 'all 0.03s ease-out' }}
              >
                {note}
              </text>
              {/* Show drum name below when intensity > 0.3 */}
              {hasDrumMapping && intensity > 0.3 && (
                <text
                  x={pos.x}
                  y={pos.y + 22}
                  textAnchor="middle"
    ...
    <div className="chord-detector" data-testid="chord-display">
      ...
      {showDiagram && (
        <div className="chord-diagram" data-testid="chord-diagram">
          <div className="dual-circles-with-info">
            {/* Left Info Panel - Recent Chords */}
            <div className="circle-info-panel left-panel">
              <h5>Recent Chords</h5>
              <div className="chord-history-vertical">
                {groupedChords.slice(-5).reverse().map((group, idx) => (
                  <div
                    key={idx}
                    className={`chord-history-item ${idx === 0 ? 'current' : ''}`}
                    style={{
                      borderLeftColor: getChordColor(group.chord),
                      opacity: 1 - (idx * 0.15)
                    }}
                  >
                    <span className="chord-name">{group.chord}</span>
                    {detectedKey && (
                      <span className="nashville">{toNashvilleNumber(group.chord, detectedKey)}</span>
                    )}
                  </div>
                ))}
                {groupedChords.length === 0 && (
                  <div className="no-data">No chords detected</div>
                )}
              </div>
              {detectedKey && (
            ...
            {/* Right Info Panel - Rhythm Pattern */}
            <div className="circle-info-panel right-panel">
              <h5>Rhythm Pattern</h5>
              <div className="drum-activity-list">
                {Object.entries(DRUM_COLORS).map(([drumType, color]) => {
                  const intensity = drumIntensities[drumType] || 0;
                  const hitCount = drumHits?.[drumType]?.length || 0;
                  return (
                    <div
                      key={drumType}
                      className={`drum-activity-item ${intensity > 0 ? 'active' : ''}`}
                      style={{
                        borderLeftColor: color,
                        backgroundColor: intensity > 0 ? `${color}22` : 'transparent'
                      }}
                    >
                      <span className="drum-name" style={{ color: intensity > 0 ? color : '#666' }}>
                        {drumType.charAt(0).toUpperCase() + drumType.slice(1)}
                      </span>
                      <span className="drum-hits">{hitCount}</span>
                      <div className="drum-meter">
                        <div
                          className="drum-meter-fill"
                          style={{
                            width: `${intensity * 100}%`,
