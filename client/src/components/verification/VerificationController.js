...
export function VerificationController({
  ...
  // Track previous chord count to detect new chords
  const chordCount = chordData?.chordHistory?.length || 0;

  useEffect(() => {
    const chordStatus = stageStatus[STAGES.CHORD];
    const hasChordData = chordData && chordData.chordHistory && chordData.chordHistory.length > 0;

    // Run verification if:
    // 1. We have chord data with at least one chord
    // 2. Stage is not BLOCKED and not already VERIFIED
    // This allows verification to run/update when new chords are detected
    if (
      hasChordData &&
      chordStatus !== STAGE_STATUS.BLOCKED &&
      chordStatus !== STAGE_STATUS.VERIFIED
    ) {
      runChordVerification(chordData);
    }
  }, [chordData, chordCount, stageStatus, runChordVerification]);
